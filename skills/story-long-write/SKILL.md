...
- **追踪/信息权限.md**：秘密/金手指/身份/阴谋的信息权限表，记录“谁知道、谁不知道、何时公开”。凡主角私人金手指、隐藏身份、未来计划、反派暗线，都必须登记。
...
- **追踪/伏笔.md** + **追踪/时间线.md** + **追踪/信息权限.md**：伏笔状态表+故事时间线+秘密知情边界（参考 plot-core-methods.md「连续性追踪」）
...
| 追踪/信息权限.md | 全书 | Phase 2 起 | 每章写作前、对话/揭示/反派视角前 |
...
2. **读取上下文**（按需加载，缺失则跳过。Codex 使用 `rg` 和文件读取直接加载，不依赖 Claude agent 定义）：
   ...
   - (4) `追踪/信息权限.md`（如存在）— 本章所有秘密/金手指/隐藏身份的知情边界
   - (5) `设定/角色/{相关角色}.md` — 本章涉及角色
   - (6) `对标/{对标书名}/拆文报告.md`（如存在）— 对标参考（如不存在，查找 `拆文库/{对标书名}/拆文报告.md`）
   - (7) `对标/{对标书名}/原文/第{N}章_*.md`（如存在）— 同位置章节参考
   - (8) `参考资料/{topic}.md`（如存在）— 历史研究资料（由 story-researcher 产出）
3. **确认节奏**：本章是快节奏（冲突/打斗）还是慢节奏（铺垫/日常）
3.2. **信息权限预检**：写正文前列出本章出场角色的“可说信息/不可说信息/只能误判的信息”。如果角色不知道主角金手指、隐藏身份、幕后真相，就禁止让该角色直接说出准确名称、机制、来源、数值或结论。角色只能基于可见现象做错误或不完整推断。
...
6. **检查**：章尾是否有钩子、爽点是否到位；执行信息权限检查，确认没有角色越权知道主角私人金手指、隐藏身份、未来计划或幕后真相
7. **禁用词扫描**：对照 `references/banned-words.md` 检查本章，一级词（高频AI腔）命中即替换；二级词（低频/语境相关）高频出现时替换，偶发可参考 `references/anti-ai-writing.md` 定性裁定
8. **更新追踪**：写完后即时更新 `追踪/伏笔.md`（新增/回收伏笔）、`追踪/时间线.md`（记录事件时序）和 `追踪/信息权限.md`（新增知情者/误判者/公开信息）
...
提前披露是高优先级错误。除非 `追踪/信息权限.md` 或正文已明确让该角色得知，否则禁止以下写法：

- 配角直接说出主角私人金手指的准确名称、机制、数值、来源或使用条件
- 反派/路人准确判断主角隐藏身份、穿越、系统、面板、异力炉等私密设定
- 对话中出现“你有系统吧”“你的面板显示”“你靠异力炉炼化”这类超出角色认知的精准台词
- 旁白用非主角限知视角替配角确认秘密

允许的替代写法：
- **可见现象**：角色只能说看到的结果，如“你刚才力气不对”“那黑纹又亮了”
- **错误推断**：角色可以误判，如“你是不是练了邪功”“你背后有人”
- **模糊怀疑**：角色可以怀疑但不能定名，如“你身上有东西”
- **试探套话**：角色用问题逼主角露破绽，但问题必须来自已有线索
...
信息权限泄漏按严重度处理：
- S1：角色说出其无从得知的金手指/隐藏身份/幕后真相准确名称或机制，必须改
- S2：旁白或对话暗示过准，等同提前剧透，需要改为可见现象或错误推断
- S3：读者能猜到但角色表达过直，需要弱化
