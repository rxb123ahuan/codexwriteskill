...
**Codex 上下文加载**。使用 `rg` 和文件读取批量加载写作上下文。不要依赖 Claude agent 定义；只有用户明确要求并行/子代理时，才可将只读查询拆给 Codex subagent。
...
2. **逐章执行**：
   - 读细纲 → 按需加载角色设定 → 写正文 → **字数验证（`wc -m`统计，< 目标90%则强制扩充）** → 检查钩子/爽点 → 禁用词扫描
   - 每章写完后**立即更新**以下文件：
     - `追踪/伏笔.md`（新增/回收伏笔）
     - `追踪/时间线.md`（记录事件时序）
     - `追踪/上下文.md`（更新进度摘要）
...
**资料研究（按需）**：如果写作中遇到需要查证的外部事实（历史年代、地理方位、职业细节等），暂停写作，按 Codex 当前搜索/浏览规则核验并输出到 `参考资料/` 目录。研究完成后再继续写作。
